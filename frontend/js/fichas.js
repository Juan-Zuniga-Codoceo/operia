...
createApp({
  setup() {
    ...
    const showEditModal = ref(false); // <-- AÑADIDO
    const sheetToEdit = ref({}); // <-- AÑADIDO
    ...
    const filteredSheets = computed(() => {
      return allSheets.value.filter(sheet => {
        const term = searchTerm.value.toLowerCase(); // <-- Definido term
        const searchMatch = !searchTerm.value ||
          sheet.product_name.toLowerCase().includes(term) ||
          (sheet.model && sheet.model.toLowerCase().includes(term)) ||
          (sheet.tags && sheet.tags.toLowerCase().includes(term)) ||
          (sheet.sku && sheet.sku.toLowerCase().includes(term)); // <-- AÑADIDO SKU
    ...
    const downloadSheet = (sheet) => {
      ...
      // Usamos la URL base de api.js para construir la URL completa
      const downloadUrl = `${API_BASE_URL}/api/sheets/${sheet.id}/download`;
    ...
    const previewSheet = async (sheet) => {
      try {
        // Usamos el nuevo método de api.js
        const blob = await API.requestBlob(`/api/sheets/${sheet.id}/preview`);

        // Creamos una URL temporal para el blob
        const fileURL = window.URL.createObjectURL(blob);

        // Abrimos esa URL en una nueva pestaña
        ...
        API.showNotification('No se pudo generar la vista previa.', 'error');
      }
    };
    ...
    const uploadSheet = async () => {
      ...
      formData.append('sku', newSheet.value.sku); // <-- AÑADIDO
    ...
    const saveSheetChanges = async () => {
      if (!sheetToEdit.value.product_name.trim()) {
        return API.showNotification('El nombre del producto es requerido.', 'warning');
      }
      isUploading.value = true; // Re-usamos el estado de 'cargando'
      try {
        const { id, ...dataToUpdate } = sheetToEdit.value;
        // Limpiamos datos que no se deben enviar
        delete dataToUpdate.file_name;
        delete dataToUpdate.file_path;
        delete dataToUpdate.uploaded_at;
        delete dataToUpdate.uploaded_by_name;
        delete dataToUpdate.category_name;

        await API.put(`/api/sheets/${id}`, dataToUpdate);
        API.showNotification('Ficha actualizada.', 'success');
        closeEditModal();
        await loadData(); // Recarga los datos
      } catch (error) {
        API.showNotification(error.message || 'No se pudo actualizar.', 'error');
      } finally {
        isUploading.value = false;
      }
    };
    ...
    const openUploadModal = () => {
      newSheet.value = { product_name: '', model: '', category_id: '', tags: '', sku: '', sheetFile: null }; // <-- AÑADIDO SKU
      newCategoryName.value = '';
    ...
    const openEditModal = (sheet) => {
      sheetToEdit.value = { ...sheet }; // Copia la ficha para editar
    ...
    const closeModalOnSelf = (event, modalName) => {
      if (event.target === event.currentTarget) {
        if (modalName === 'showUploadModal') closeUploadModal();
        if (modalName === 'showEditModal') closeEditModal();
      }
    };
