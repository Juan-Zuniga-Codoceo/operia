// Script para poblar Operia con datos de prueba realistas para video publicitario
...
console.log('🎬 Iniciando población de datos demo para video Operia...\n');
...
async function seedData() {
    ...
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            ...
            USERS.forEach(user => {
                userStmt.run(user.name, user.email, defaultPassword, user.office, user.role);
            });
            userStmt.finalize();
            ...
            const clientStmt = db.prepare(
                `INSERT OR IGNORE INTO clients (rut, name, email, phone, address_street, commune, region, reference) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
            );
            ...
            CLIENTS.forEach(client => {
                clientStmt.run(
                    ...
                    client.address_street, client.commune, client.region, client.reference
                );
            });
            clientStmt.finalize();
            ...
            const labelStmt = db.prepare(
                `INSERT OR IGNORE INTO labels (name, color, created_by) VALUES (?, ?, 1)`
            );

            LABELS.forEach(label => {
                labelStmt.run(label.name, label.color);
            });
            labelStmt.finalize();
            console.log(`✅ ${LABELS.length} etiquetas creadas\n`);

            // Obtener IDs de usuarios y clientes creados
            db.all('SELECT id FROM users ORDER BY id', (err, users) => {
                if (err) {
                    reject(err);
                    return;
                }
                ...
                db.all('SELECT id FROM clients ORDER BY id', (err, clients) => {
                    ...
                    const taskStmt = db.prepare(
                        `INSERT INTO tasks (title, description, priority, status, created_by, due_date, origin, created_at) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
                    );

                    TASKS_INTERNAL.forEach(task => {
                    ...
                    const clientTaskStmt = db.prepare(
                        `INSERT INTO tasks 
             (title, description, priority, status, created_by, due_date, origin, shipping_type, payment_status, client_snapshot, created_at) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
                    );

                    TASKS_CLIENTS.forEach(task => {
                        const createdBy = users[Math.floor(Math.random() * users.length)].id;
                        ...
                        clientTaskStmt.run(
                            task.title, task.description, task.priority, task.status,
                            createdBy, dueDate, task.origin, task.shipping_type,
                            task.payment_status, clientSnapshot, createdAt
                        );
                    });
                    clientTaskStmt.finalize();
                    ...
                    db.all('SELECT id FROM tasks', (err, tasks) => {
                        ...
                        tasks.forEach(task => {
                            ...
                            for (let i = 0; i < numAssignments && i < shuffledUsers.length; i++) {
                                assignStmt.run(task.id, shuffledUsers[i].id);
                            }
                        });
                        assignStmt.finalize();
                        ...
                        db.all('SELECT id FROM labels', (err, labels) => {
                            if (err) {
                                reject(err);
                                return;
                            }

                            const labelAssignStmt = db.prepare(
                            ...
                            tasks.forEach(task => {
                                ...
                                for (let i = 0; i < numLabels && i < shuffledLabels.length; i++) {
                                    labelAssignStmt.run(task.id, shuffledLabels[i].id);
                                }
                            });
                            labelAssignStmt.finalize();
